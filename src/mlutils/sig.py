...
def generate_signatures(dbname, table, infile=None, outfile=None, models_folder=None, plots_folder=None, sig_stats_file=None,
    ...
    if models_folder is None:
        models_folder = coordinate2mosaic(
            coord) + "/" + checkerconfig.DEFAULT_MODELS_FOLDER + "/sig"

    if plots_folder is None:
        plots_folder = coordinate2mosaic(
            coord) + "/" + checkerconfig.DEFAULT_PLOTS_FOLDER
    ...
    if table in checkerconfig.CHEM_TABLES:
        ...
        for t in tqdm_local(log, tables):
            mymols.update(Psql.fetch_inchikeys(t, dbname))
    ...
    if table not in checkerconfig.CONTINUOUS_TABLES:
        ...
        if table in checkerconfig.CHEM_TABLES:
            ...
            if not infile:
                ...
                for r in tqdm_local(log, cur):
                    if r[0] not in mymols or not r[1]:
                        continue
                    f.write("%s %s\n" % (r[0], r[1]))
                    InitMols += 1
        ...
        if not recycle or not os.path.exists(models_folder + "/dictionary.pkl"):
            ...
            dictionary = corpora.Dictionary(l.rstrip("\n").split(" ")[1].split(
                ",") for l in tqdm_local(log, open(plain_corpus, "r")))
        ...
        if not recycle or not os.path.exists(models_folder + "/cut.txt"):
            ...
            exp_var_ratios = lsi_variance_explained(
                tfidf_corpus, lsi, B=B, N=N, num_topics=num_topics, log=log)

            cut_i, elb_i = variance_plot(
                exp_var_ratios, table=table, variance_cutoff=variance_cutoff, plot_folder=plots_folder)
    ...
    # PCA
        ...
        if table == "physchem":
            ...
            if not infile:
                R = Psql.qstring(
                    "SELECT inchikey, raw FROM %s WHERE raw IS NOT NULL" % table, dbname)
                RowNames = []
                X = []
                for r in tqdm_local(log, R):
                    RowNames += [r[0]]
        ...
        if not recycle or not os.path.exists(models_folder + "/cut.txt"):
            ...
            cut_i, elb_i = variance_plot(
                pca.explained_variance_ratio_, table=table, variance_cutoff=variance_cutoff, plot_folder=plots_folder)
            with open(models_folder + "/cut.txt", "w") as f:
                f.write("%d\n%d\n" % (cut_i, elb_i))
    ...
    if not recycle or not os.path.exists(models_folder + "/bg_distances.h5"):
        ...
        with h5py.File(models_folder + "/bg_distances.h5", "a") as hf:
            ...
            if "distance" not in hf.keys() or "pvalue" not in hf.keys():
                ...
                pvals = distance_background(
                    inchikey_sig, inchikeys, B=B_distances)
                inchikey_sig.close()
    ...
    inchikey_sig = shelve.open(tmp + ".dict", "r")
    ks_moa, auc_moa = vector_validation(
        inchikey_sig, "sig", table, prefix="moa", plot_folder=plots_folder, files_folder=filesdir)
    ks_atc, auc_atc = vector_validation(
        inchikey_sig, "sig", table, prefix="atc", plot_folder=plots_folder, files_folder=filesdir)
    inchikey_sig.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--db', type=str, help='Chemical Checker database name', required=True)
    parser.add_argument('--table', type=str,
                        help='Chemical Checker table name', required=True)
    parser.add_argument('--infile', default=None, type=str,
                        help='Raw signature file (should coincide with formats available in the database)')
    parser.add_argument('--outfile', default=None, type=str,
                        help='Output file with extension .h5')
    parser.add_argument('--models_folder', default=None,
                        type=str, help='Models folder')
    parser.add_argument('--plots_folder', default=None,
                        type=str, help='Plots folder')
    parser.add_argument('--sig_stats_file', default=None,
                        type=str, help='Signature stats file')
    parser.add_argument('--min_freq', default=5, type=int,
                        help='Minimum frequency (counts)')
    parser.add_argument('--max_freq', default=None, type=float,
                        help='Maximim frequency (proportion')
    parser.add_argument('--num_topics', default=None, type=int,
                        help='Number of topics to start with')
    parser.add_argument('--B', default=10, type=int,
                        help='In the variance explained, number of random passes')
    ...
    parser.add_argument('--recycle', default=False,
                        action='store_true', help='Recycle stored models')
    parser.add_argument('--variance_cutoff', default=0.9,
                        type=float, help='Variance cutoff')
    parser.add_argument('--integerize', default=False, action='store_true',
                        help='Save an integerized (binned) version of the matrices')
    parser.add_argument('--not_normalized', default=False,
                        action='store_true', help='Do not normalize the vectors')
    parser.add_argument('--filesdir', default=None, type=str,
                        help="Where validation files are stored")
