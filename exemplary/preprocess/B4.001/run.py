...
def parse_bindingdb(ACTS=None, bindingdb_file=None):
    ...
    bdlig_inchikey = {}
    inchikey_inchi = {}
    molrepos = Molrepo.get("bindingdb")
    for molrepo in molrepos:
        if not molrepo.inchikey:
            continue
...
def main():
    ...
    dataset = 'B4.001'  # os.path.dirname(os.path.abspath(__file__))[-6:]
    ...
    bindingdb_file = os.path.join(map_files["binding"], "full database.xml")
