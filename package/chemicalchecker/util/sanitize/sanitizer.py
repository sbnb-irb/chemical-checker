...
class Sanitizer(object):
    ...
    def __init__(self, trim, max_keys=1000, max_features=5000, min_feature_freq=5, max_feature_freq=0.8, min_key_freq=1, max_key_freq=0.8, chunk_size=10000):
        """Initialize
    ...
    def boolean_matrix(self, data):
        ...
        with h5py.File(data, "a") as hf:
            if "B" in hf.keys():
                self.__log.warn("There was a B (boolean) dataset. It has been removed.")
                del hf["B"]
    ...
    def transform(self, data=None, V=None, keys=None, keys_raw=None, features=None, sign=None):
        """Sanitize data
        ...
        if data is None:
            ...
            if V is None or keys is None or keys_raw is None or features is None:
                raise Exception("There is no data or V, keys, keys_raw, features provided...")
            tag = str(uuid.uuid4())
            ...
            with h5py.File(data, "a") as hf:
                ...
                for chunk in self.chunker(n):
                    ...
                    if create:
                        ...
                        hf[name_tmp].resize((hf[name_tmp].shape[0]+M_tmp.shape[0]), axis=0)
        ...
        with h5py.File(data, "a") as hf:
            ...
            for chunk in self.chunker(n):
                ...
                name_tmp = "V_tmp"
                if create:
                    hf.create_dataset(name_tmp, data=V, maxshape=(None, V.shape[1]))
                    create = False
                else:
                    hf[name_tmp].resize((hf[name_tmp].shape[0]+V.shape[0]), axis=0)
