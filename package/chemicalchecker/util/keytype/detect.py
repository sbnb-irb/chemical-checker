"""Automatically detect type of a list of keys. Homogeneity of key type across the list is assumed."""
...
class KeyTypeDetector(object):
    ...
    def is_inchikey(self, key):
        if len(key) != 27: return False
        if key[25] != "-": return False
        if key[14] != "-": return False
        if not key[:14].isalnum(): return False
        if not key[15:25].isalnum(): return False
        if not key[-1].isalnum(): return False
        return True

    def is_smiles(self, key):
