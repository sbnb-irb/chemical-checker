...
@logged
class SmilesTripletTraintest(object):
    ...
    def precomputed_triplets(X, triplets, out_file,
        ...
        split_idxs = SmilesTripletTraintest.get_split_indeces(
        ...
        '''
        # create dataset
        SmilesTripletTraintest.__log.info('Traintest saving to %s', out_file)
        ...
        SmilesTripletTraintest.__log.info(
            'SmilesTripletTraintest saved to %s', out_file)

    @staticmethod
    def create(X, out_file, neigbors_sign, check_distances=True,
               split_names=['train', 'test'], split_fractions=[.8, .2],
               suffix='eval', x_dtype=np.float32, y_dtype=np.float32,
               num_triplets=1e6, cpu=1):
        ...
        SmilesTripletTraintest.__log.debug(
            "{:<20} shape: {:>10}".format("input X", str(X.shape)))

        # reduce redundancy, keep full-ref mapping
        SmilesTripletTraintest.__log.info("Reducing redundancy")
        ...
        medi_triplet_per_mol = max([int(np.ceil(triplet_per_mol * 0.15)), 1])#triplet_per_mol - (2 * easy_triplet_per_mol)
        hard_triplet_per_mol = max([triplet_per_mol - (easy_triplet_per_mol + medi_triplet_per_mol), 1])#easy_triplet_per_mol
        SmilesTripletTraintest.__log.info("Triplet_per_mol: %s" % triplet_per_mol)
        SmilesTripletTraintest.__log.info("E triplet per mol: %s" % easy_triplet_per_mol)
        SmilesTripletTraintest.__log.info("M triplet per mol: %s" % medi_triplet_per_mol)
        SmilesTripletTraintest.__log.info("H triplet per mol: %s" % hard_triplet_per_mol)
        SmilesTripletTraintest.__log.info("Triplet_per_mol: %s" % (easy_triplet_per_mol + medi_triplet_per_mol + hard_triplet_per_mol))
        ...
        split_chunk_idx = SmilesTripletTraintest.get_split_indeces(
        ...
        SmilesTripletTraintest.__log.info('Generating NN indexes')
        NN = dict()
        ...
        with h5py.File(out_file, "w") as fh:
            ...
            for split1, split2 in combos:
                ...
                SmilesTripletTraintest.__log.debug("SPLIT: %s" % combo)
                # define F and T according to the split that is being used
                T = 10
                F = 200
                SmilesTripletTraintest.__log.info("F and T: %s %s" % (F, T))
                assert(T < F)

                SmilesTripletTraintest.__log.info("Searching Neighbors")
                ...
                SmilesTripletTraintest.__log.info("Generating triplets")
                ...
                # get reference ids
                SmilesTripletTraintest.__log.info("Mapping triplets")
                ...
                # stack triplets
                SmilesTripletTraintest.__log.info("Stacking triplets")
                ...
                if check_distances:
                    ...
                    SmilesTripletTraintest.__log.warning(
        ...
        if check_distances:
            ...
            plot_file = os.path.join(
                os.path.split(out_file)[0], 'sign3_dists_%s.png' % suffix)
    ...
    @staticmethod
    def generator_fn(file_name, split, sign, epochs=None, batch_size=None, 
        shuffle=True, sharedx=None, train=True):
        ...
        """
        reader = SmilesTripletTraintest(file_name, split)
        reader.open()
        # read shapes
        ...
        if sharedx is not None:
            ...
            SmilesTripletTraintest.__log.debug('Reading X in memory')
            X = reader.get_all_x()
        SmilesTripletTraintest.__log.debug('X shape: %s' % str(X.shape))
        ...
        SmilesTripletTraintest.__log.debug('Generator ready')
        ...
        sign_shape = (t_shape[0], sign.shape[1])
        label_shape = (t_shape[0],)
        shapes = (triplet_shape, triplet_shape, triplet_shape, sign_shape, label_shape)
        dtypes = (x_dtype, x_dtype, x_dtype, x_dtype, x_dtype)
