...
class ConvMol(object):
    ...
    def get_null_mol(n_feat, max_deg=10, min_deg=0):
        ...
        """
        # Use random insted of zeros to prevent weird issues with summing to
        # zero
    ...
    def agglomerate_mols(mols, max_deg=10, min_deg=0):
        ...
        # Mergesort is a "stable" sort, so the array maintains it's secondary
        # sort of mol_index
        all_atoms = atoms_by_deg[degree_vector.argsort(kind='mergesort')]

        # Sort all atoms by degree.
        # Get the size of each atom list separated by molecule id, then by
        # degree
        ...
        start_by_deg = np.vstack(
            [cumulative_sum_minus_last(l) for l in mol_deg_sz])
        ...
        # This is used for convience in the following function (explained
        # below)
        ...
        def to_final_id(mol_atom_id, mol_id):
            ...
            # representation (first term). The sum of the two is the final
            # indexn
        ...
        for deg in range(min_deg, max_deg + 1):
            ...
            for mol_id in range(num_mols):
                # Get the adjacency lists for this molecule and current degree
                # id
                ...
                for i in range(nbr_list.shape[0]):
                    for j in range(nbr_list.shape[1]):
                        deg_adj_lists[deg_id][row, j] = to_final_id(
                            nbr_list[i, j], mol_id)
