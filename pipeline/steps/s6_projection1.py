...
class Projection1(BaseStep):
    ...
    def run(self):
        ...
        for ds in all_datasets:
            if not ds.essential:
                continue
            sign1 = cc.get_signature("proj1", "full", ds.dataset_code)
            if sign1.is_fit():
                continue
        ...
        job_path = tempfile.mkdtemp(
            prefix='jobs_proj1_', dir=self.tmpdir)
        ...
        script_lines = [
            ...
            "pars = {'cpu': 10}",
            # start import
            'proj1_ref = cc.get_signature("proj1", "reference", data,**pars)',
            "proj1_ref.fit(sign1_ref)",
            "proj1_full = cc.get_signature('proj1', 'full', data,**pars)",
            "proj1_ref.predict(sign1_full, destination=proj1_full.data_path)",
            "proj1_full.mark_ready()",
            "print('JOB DONE')"
        ]

        script_name = os.path.join(job_path, 'proj1_script.py')
        ...
        for code in dataset_codes:
            ...
            sign0 = cc.get_signature("proj1", "full", code)
            if sign0.is_fit():
                continue

            dataset_not_done.append(code)
            self.__log.warning(
                "Projection1 fit failed for dataset code: " + code)
