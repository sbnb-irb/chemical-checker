...
def fetch_binding(any_human, ACTS_binding, prot_allgos):

    ACTS = collections.defaultdict(list)
    for k, v in ACTS_binding.items():
        ik = k[0]
        uniprot_ac = k[1]
        act = v
    ...
    def get_allgos(uniprot_ac):
        if not os.path.exists("%s/%s.tsv" % (prot_allgos, uniprot_ac)):
            return None
        gos = set()
...
def main():
    ...
    goa_file = os.path.join(
        map_files["goa_human"], "goa_human.gaf")
    go_file = os.path.join(
        map_files["gobasic"], "go-basic.obo")

    tmpdir = tempfile.mkdtemp(
        prefix='sign0_' + dataset_code + "_", dir=Config().PATH.CC_TMP)

    main._log.debug("Temporary files saved in " + tmpdir)
    ...
    main._log.info(" Submit job bpanno for the uniprots " + str(len(uniprots)) + " found")

    # Main
    prot_allgos = os.path.join(tmpdir, "prot_allgos")

    if os.path.exists(prot_allgos) is False:
        os.makedirs(prot_allgos)

    job_path = os.path.join(Config().PATH.CC_TMP, "job_bpanno")
    ...
    script_name = myfold + "/prot2allgos.py"

    params = {}

    params["num_jobs"] = len(uniprots) / 20
    params["jobdir"] = job_path
    params["job_name"] = "CC_C4_bpanno"
    params["elements"] = list(uniprots)
    params["memory"] = 10
    # job command
    singularity_image = Config().PATH.SINGULARITY_IMAGE
    command = "singularity exec {} python {} <TASK_ID> <FILE> {} {}".format(
        singularity_image, script_name, tmpdir, goa_file)
    # submit jobs
    cluster = HPC(Config())
    ...
    if cluster.status() == 'error':
        main._log.error(
            "Bpanno job produced some errors. The preprocess script can't continue")
        sys.exit(1)
    ...
    GOS = fetch_binding(any_human, ACTS, prot_allgos)
