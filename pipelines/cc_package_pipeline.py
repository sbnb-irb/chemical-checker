...
from chemicalchecker.core import Diagnosis
...
def downloads(tmpdir):
    ...
    # (Nico) Generate the Chembl files drugtargets and drugindications via the
...
c1_task = CCFit(CC_ROOT, 'clus1', **c1_params)
pp.add_task(c1_task)

# TASK: Calculate nearest neighbors for signatures 1
n1_params = {}
n1_task = CCFit(CC_ROOT, 'neig1', **n1_params)
pp.add_task(n1_task)

# TASK: Calculate projections for signatures 1
p1_params = {}
p1_task = CCFit(CC_ROOT, 'proj1', **p1_params)
pp.add_task(p1_task)

# TASK: Calculate signatures 2
s2_params = {}
s2_task = CCFit(CC_ROOT, 'sign2', **s2_params)
pp.add_task(s2_task)

# TASK: Calculate nearest neighbors for signatures 2
n2_params = {}
n2_task = CCFit(CC_ROOT, 'neig2', **n2_params)
pp.add_task(n2_task)

# TASK: Calculate projections for signatures 2
p2_params = {}
p2_task = CCFit(CC_ROOT, 'proj2', **p2_params)
pp.add_task(p2_task)
...
s3_params = {}
s3_task = CCFit(CC_ROOT, 'sign3', **s3_params)
pp.add_task(s3_task)
